"""Build the isolated full Phase 2 dataset from its frozen science-first frame ledger."""
...
def matched_negative(index, timestamp, target_lat, target_lon, image, config, rng, forbidden_bounds=None):
    mask, sat, labels, sampling = config["study_mask"], config["satellite"], config["labels"], config["full_build"]["sampling"]
    ...
    for level, (dy, dx) in enumerate(offsets):
        ...
        for _ in range(200):
            ...
            if forbidden_bounds is not None and bounds in forbidden_bounds:
                continue
...
def row_record(frame, files, timestamp, label, reason, centre_lat, centre_lon, x, y, bounds, patch, digest, path, config,
    ...
    return {"path": str(path), "label": label, "split": frame.split, "frame_id": f"H09_{timestamp.strftime('%Y%m%d_%H%M')}",
            ...
            "configuration_hash": config["full_build"]["configuration_hash"], "schema_version": config["software_schema"]["dataset_schema_version"], "label_rule_version": config["labels"]["label_rule_version"]}


def build(config_path: Path, allow_download: bool = True, resume: bool = True) -> tuple[pd.DataFrame, dict]:
    started = time.perf_counter(); config = yaml.safe_load(config_path.read_text(encoding="utf-8")); phase = config["full_build"]
    ...
    rng = np.random.default_rng(int(phase["seeds"]["patch_sampling"])); root = Path(phase["outputs"]["root"])
    Path(phase["outputs"]["state_root"]).mkdir(parents=True, exist_ok=True)
    ...
    target_positions = {split: 0 for split in targets_by_split}; rows = []; unavailable = []; completed = []; download_records = []; downloaded_bytes = 0; downloaded_files = 0; peak = working_set_bytes()
    ...
    if resume and checkpoint_path.exists():
        ...
        if checkpoint.get("identity") != identity:
            raise RuntimeError("Phase 2 full checkpoint does not match the frozen ledger/configuration")
        start_number = int(checkpoint["next_frame_number"])
        rows = checkpoint["rows"]
        unavailable = checkpoint["unavailable"]
        completed = checkpoint.get("completed", [])
        download_records = checkpoint.get("download_records", [])
        ...
        LOGGER.info("Resuming Phase 2 full at frame %d/%d with %d samples", start_number, len(ledger), len(rows))
    for number, frame in enumerate(ledger.itertuples(), 1):
        ...
        frame_new_files = 0; frame_new_bytes = 0
        try:
            ...
            for path in files:
                if before[str(path)] is None:
                    downloaded_files += 1; downloaded_bytes += path.stat().st_size; frame_new_files += 1; frame_new_bytes += path.stat().st_size
            image = load_frame(files, config)
        except Exception as exc:
            LOGGER.exception("Unavailable frozen Phase 2 full frame %s", timestamp); unavailable.append({"frame_number":number,"timestamp":timestamp.isoformat(),"frame_id":f"H09_{timestamp.strftime('%Y%m%d_%H%M')}","error":str(exc)})
            download_records.append({"frame_number":number,"timestamp":timestamp.isoformat(),"new_files":frame_new_files,"new_bytes":frame_new_bytes,"status":"failed"})
            _atomic_json(checkpoint_path,{"identity":identity,"next_frame_number":number+1,"rows":rows,"unavailable":unavailable,"completed":completed,"download_records":download_records,"downloaded_bytes":downloaded_bytes,"downloaded_files":downloaded_files,"peak_working_set_bytes":peak,"target_positions":target_positions,"rng_state":rng.bit_generator.state})
            pd.DataFrame(unavailable).to_csv(phase["outputs"]["failed_frame_ledger"],index=False); pd.DataFrame(completed).to_csv(phase["outputs"]["completed_frame_ledger"],index=False); pd.DataFrame(download_records).to_csv(phase["outputs"]["download_ledger"],index=False)
            continue
        frame_rows = 0; used_crop_keys = set()
        if frame.category == "active":
            ...
            for positive_number, strike in enumerate(strikes.itertuples()):
                ...
                negative = matched_negative(index, timestamp, strike.lat, strike.lon, image, config, rng, {bounds for bounds,label in used_crop_keys})
                if negative:
                    used_crop_keys.add((negative["bounds"], 0))
            ...
            for zero_number in range(int(sampling["zero_recorded_patches_per_frame"])):
                ...
                negative = matched_negative(index, timestamp, target[0], target[1], image, config, rng, {bounds for bounds,label in used_crop_keys})
        ...
        peak = max(peak, working_set_bytes()); completed.append({"frame_number":number,"timestamp":timestamp.isoformat(),"frame_id":f"H09_{timestamp.strftime('%Y%m%d_%H%M')}","samples":frame_rows}); download_records.append({"frame_number":number,"timestamp":timestamp.isoformat(),"new_files":frame_new_files,"new_bytes":frame_new_bytes,"status":"completed"}); LOGGER.info("Phase2 full frame %d/%d %s samples=%d", number, len(ledger), timestamp, frame_rows)
        _atomic_json(checkpoint_path, {"identity": identity, "next_frame_number": number + 1, "rows": rows,
                     "unavailable": unavailable, "completed": completed, "download_records": download_records, "downloaded_bytes": downloaded_bytes, "downloaded_files": downloaded_files,
                     "peak_working_set_bytes": peak, "target_positions": target_positions, "rng_state": rng.bit_generator.state})
        Path(phase["outputs"]["state_root"]).mkdir(parents=True,exist_ok=True); pd.DataFrame(unavailable).to_csv(phase["outputs"]["failed_frame_ledger"],index=False); pd.DataFrame(completed).to_csv(phase["outputs"]["completed_frame_ledger"],index=False); pd.DataFrame(download_records).to_csv(phase["outputs"]["download_ledger"],index=False)
    manifest = pd.DataFrame(rows); manifest_path = Path(phase["outputs"]["manifest"]); manifest_path.parent.mkdir(parents=True, exist_ok=True); temporary_manifest=manifest_path.with_suffix(".csv.tmp"); manifest.to_csv(temporary_manifest,index=False); temporary_manifest.replace(manifest_path)
    unavailable_path = root / "unavailable_frames.json"; unavailable_path.write_text(json.dumps(unavailable, indent=2) + "\n", encoding="utf-8")
    metrics = {"phase": "V2 Phase 2 full dataset", "frames_planned": len(ledger), "frames_built": int(manifest.frame_id.nunique()), "unavailable_frames": len(unavailable),
...
def main() -> None:
    parser = argparse.ArgumentParser(description=__doc__); parser.add_argument("--config", type=Path, default=Path("configs/v2_full.yaml")); parser.add_argument("--no-download", action="store_true"); parser.add_argument("--no-resume", action="store_true"); args = parser.parse_args()
