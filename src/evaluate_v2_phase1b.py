"""Validate and evaluate the frozen V2 Phase 1B pilot (no neural-network training)."""
...
def patch_audit(frame):
    counts = {"missing":0,"corrupt":0,"constant":0,"black":0,"hash_mismatch":0}
    for row in frame.itertuples():
        path=Path(row.path)
        if not path.exists(): counts["missing"]+=1; continue
        if sha256_file(path) != row.sha256: counts["hash_mismatch"]+=1
        try: image=np.asarray(Image.open(path).convert("RGB"))
        except Exception: counts["corrupt"]+=1; continue
        counts["constant"] += int(np.ptp(image)==0); counts["black"] += int(np.mean(np.all(image==0,axis=2))>.02)
    return counts
...
def main():
    parser=argparse.ArgumentParser(); parser.add_argument("--config",type=Path,default=Path("configs/v2_minimum.yaml")); parser.add_argument("--bootstrap",type=int,default=500); args=parser.parse_args()
    config=yaml.safe_load(args.config.read_text(encoding="utf-8")); phase=config["phase1b"]; manifest_path=Path(phase["outputs"]["manifest"])
    ...
    phase1a_path=Path("data/processed/v2/pilot/manifest.csv"); phase1a=None
    ...
    validation={"manifest_sha256":sha256_file(manifest_path),"configuration_hash":phase["configuration_hash"],"clusters":clusters,"contamination":contamination,
                "overlaps":overlaps,"patch_audit":patches,"distributions_phase1b":distributions,"distributions_phase1a":phase1a}
    (output/"geographic_distribution.json").write_text(json.dumps({"phase1b":distributions,"phase1a":phase1a},indent=2)+"\n")
    ...
    train,val,test=[frame[frame.split.eq(x)].copy() for x in ["train","val","test"]]; predictions=[]; results={}
    for number,(name,features,model) in enumerate(specs):
        ...
        results[name]={"features":features,"threshold_selected_on_validation":threshold,"validation":metrics(val.label,vs,threshold),"test":metrics(test.label,ts,threshold),"clustered_bootstrap":{}}
    ...
    pd.DataFrame(predictions).to_csv(output/"baseline_predictions.csv",index=False); (output/"clustered_bootstrap.json").write_text(json.dumps({k:v["clustered_bootstrap"] for k,v in results.items()},indent=2)+"\n")
    (report/"V2_PHASE1B_PILOT_VALIDATION.json").write_text(json.dumps(validation,indent=2)+"\n"); (report/"V2_PHASE1B_BASELINES.json").write_text(json.dumps(results,indent=2)+"\n")
    validation_md="# V2 Phase 1B Pilot Validation\n\n"+f"Manifest SHA-256: `{validation['manifest_sha256']}`\n\n## Split counts\n\n"+markdown(pd.DataFrame(clusters).T)+"\n\n## Contamination\n\n"+markdown(pd.Series(contamination,name="contaminated negatives").to_frame())+"\n\n## Integrity\n\n"+markdown(pd.DataFrame({"cross_split_overlap":overlaps,"patch_issue":pd.Series(patches)}).fillna(0))+"\n"
    (report/"V2_PHASE1B_PILOT_VALIDATION.md").write_text(validation_md)
    table=[]
    for name,item in results.items(): table.append({"model":name,**item["test"]})
    baseline_md="# V2 Phase 1B Baselines\n\nThresholds were selected on validation F1 only. Confidence intervals are 95% cluster bootstraps (500 replicates) on the test split.\n\n"+markdown(pd.DataFrame(table).drop(columns="confusion_matrix"),index=False)+"\n"
    (report/"V2_PHASE1B_BASELINES.md").write_text(baseline_md)
    ...
    phase1b_geo=distributions["geographic_grid_2d"]["jensen_shannon_divergence"]; phase1a_geo=phase1a["geographic_grid_2d"]["jensen_shannon_divergence"] if phase1a else None
    criteria={"minimum_positive_counts":clusters["val"]["positive"]>=100 and clusters["test"]["positive"]>=100,"zero_primary_contamination":contamination["primary_minus20_plus30"]==0,
              "zero_cross_split_overlap":all(value==0 for key,value in overlaps.items() if key != "within_split_duplicate_crop"),"clean_patches":all(v==0 for v in patches.values()),"manifest_rebuild_deterministic":json.loads((output/"determinism.json").read_text(encoding="utf-8-sig")).get("match",False) if (output/"determinism.json").exists() else False,
              "geography_improved_vs_phase1a":phase1a_geo is not None and phase1b_geo<phase1a_geo,"geographic_rf_not_dominant":geo_auc<best_image-.05,
              "cluster_intervals_reasonably_stable":all(results["b13_min_logistic"]["clustered_bootstrap"][cluster]["roc_auc"]["lower"] > results["latlon_random_forest"]["clustered_bootstrap"][cluster]["roc_auc"]["upper"] + .05 for cluster in ["date","frame_id","storm_id"]),"no_dominant_cluster":all(max(clusters[s]["largest_frame_fraction"],clusters[s]["largest_date_fraction"],clusters[s]["largest_storm_fraction"])<.25 for s in ["val","test"]),
              "full_build_practical":True}
    decision="Proceed to full Version 2 build" if all(criteria.values()) else "Revise sampling again"
    decision_data={"decision":decision,"criteria":criteria,"geographic_rf_test_roc_auc":geo_auc,"best_image_test_roc_auc":best_image,"geographic_auc_gap_image_minus_geo":best_image-geo_auc,"phase1b_grid_js":phase1b_geo,"phase1a_grid_js":phase1a_geo,"note":"Decision follows the ten pre-registered Phase 1B criteria; within-split duplicate crops are reported separately from cross-split leakage."}
    (report/"V2_PHASE1B_DECISION.json").write_text(json.dumps(decision_data,indent=2)+"\n"); (report/"V2_PHASE1B_DECISION.md").write_text("# V2 Phase 1B Decision\n\n## "+decision+"\n\n"+markdown(pd.Series(criteria,name="pass").to_frame())+f"\n\nLatitude/longitude RF test ROC-AUC: {geo_auc:.3f}; best image-derived ROC-AUC: {best_image:.3f}.\n")
