...
def compute_metrics(predictions, labels, threshold=0.5):
    ...
    po = accuracy
    expected = ((tp + fn) / len(labels)) * ((tp + fp) / len(labels)) + \
               ((tn + fp) / len(labels)) * ((tn + fn) / len(labels))
    hss = (po - expected) / (1 - expected) if (1 - expected) != 0 else 0
...
def plot_confusion_matrix(predictions, labels, results_dir='results/', threshold=0.5):
...
def error_analysis(predictions, labels, images, results_dir='results/', num_examples=10):
    """
    ...
    Args:
        ...
        images (np.ndarray): Image tensors (optional)
