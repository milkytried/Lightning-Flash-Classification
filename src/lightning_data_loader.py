...
Uses strike metadata (latitude, longitude, amplitude) as features.
"""
...
class LightningMetadataDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Returns:
            (features, label) where features are [lat, lon, amplitude, strike_type_encoded]
        """
        ...
        with h5py.File(self.hdf5_path, 'r') as f:
            ...
            # Create feature vector: [lat, lon, amplitude, strike_type]
...
def create_lightning_loaders(hdf5_path: str, batch_size: int = 512) -> dict:
    """Create train/val/test DataLoaders for lightning dataset."""
    
    train_dataset = LightningMetadataDataset(hdf5_path, split='train')
    val_dataset = LightningMetadataDataset(hdf5_path, split='val')
    test_dataset = LightningMetadataDataset(hdf5_path, split='test')
